...
public sealed class IgnoredUpdatesService
{
    ...
    private void Load()
    {
        ...
        try
        {
            ...
            var data = JsonSerializer.Deserialize<Dictionary<string, string>>(json, WinProvisionJsonOptions.Compact);
            if (data is not null)
            {
    ...
    private void Save()
    {
        try
        {
            ...
            File.WriteAllText(_filePath, JsonSerializer.Serialize(_ignored, WinProvisionJsonOptions.Compact));
        }
        catch (IOException)
        {
