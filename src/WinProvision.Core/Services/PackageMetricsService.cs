...
public sealed class PackageMetricsService
{
        ...
        try
        {
            ...
            if (File.Exists(_cachePath))
            {
                try
                {
                    ...
                    var data = JsonSerializer.Deserialize<Dictionary<string, PackageMetric>>(json, WinProvisionJsonOptions.Compact);
                    if (data is not null)
                    {
    ...
    private async Task SaveAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            string json = JsonSerializer.Serialize(_cache, WinProvisionJsonOptions.Compact);
