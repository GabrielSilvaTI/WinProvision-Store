...
public sealed record InstalledPackage(
    ...
    WingetMethod InstallationMethod = WingetMethod.Unknown);
...
public sealed class InstalledPackagesService
{
                ...
                if (completed == comTask)
                {
                    ...
                    if (comPackages.Count > 0)
                    {
                        ...
                        try
                        {
                            ...
                            var merged = MergeWithArpEntries(comPackages.Concat(cliPackages));
                            WinProvisionLog.Write(
                                $"INSTALLED LIST MERGED com={comPackages.Count} cli={cliPackages.Count} total={merged.Count}");
                            return merged;
                        }
                        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
                        {
                        ...
                        {
                            WinProvisionLog.Write($"INSTALLED LIST CLI merge indisponível; mantendo COM {ex.GetType().Name}: {ex.Message}");
                            return MergeWithArpEntries(comPackages);
                        }
                    }
    ...
    private static IReadOnlyList<InstalledPackage> MergeWithArpEntries(IEnumerable<InstalledPackage> packages)
    {
        ...
        var arpPackages = ReadUninstallEntries().Select(entry => new InstalledPackage(
            entry.DisplayName,
            $"ARP\\{entry.Hive}\\{entry.View}\\{entry.Key}",
            entry.DisplayVersion,
            "Programas instalados",
            string.Empty,
            string.Empty));
        var merged = MergeInstalledPackages(packageList, arpPackages.ToArray());
        WinProvisionLog.Write($"INSTALLED LIST ARP merged={merged.Count - packageList.Length}");
        return merged;
    }

    private static IReadOnlyList<InstalledPackage> MergeInstalledPackages(
        IReadOnlyList<InstalledPackage> comPackages,
        IReadOnlyList<InstalledPackage> cliPackages)
    {
        // Keep distinct versions and sources, while avoiding a duplicate when COM and
        // CLI describe the same installed package. Prefer COM metadata for that match.
        var merged = new List<InstalledPackage>(comPackages.Count + cliPackages.Count);
        var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (var package in comPackages.Concat(cliPackages))
        {
            string key = $"{package.Id.Trim()}|{package.Version.Trim()}|{package.Source.Trim()}";
            if (seen.Add(key))
                merged.Add(package);
        }
        return merged;
    ...
    private static async Task<IReadOnlyList<InstalledPackage>> ListCliIncludingMsStoreAsync(
        CancellationToken cancellationToken)
    {
        var packages = await ListCliAsync(cancellationToken);
        try
        {
        ...
        {
            // A fonte pode estar desativada ou indisponível; mantém a listagem normal.
            WinProvisionLog.Write($"INSTALLED LIST msstore indisponível {ex.GetType().Name}: {ex.Message}");
        }

        return packages;
    }

    private static async Task<IReadOnlyList<InstalledPackage>> ListCliAsync(
        CancellationToken cancellationToken,
        string? source = null)
    {
        var info = new ProcessStartInfo
        {
            ...
            Arguments = $"list --disable-interactivity --accept-source-agreements{(string.IsNullOrWhiteSpace(source) ? string.Empty : $" --source {source}")}",
        ...
        if (!string.IsNullOrWhiteSpace(source))
            rows = rows.Select(package => package with { Source = source }).ToList();
        WinProvisionLog.Write($"INSTALLED LIST CLI source={source ?? "default"} count={rows.Count}");
        return rows;
    }
        ...
        await Task.WhenAll(source.Select(async (package, index) =>
        {
            ...
            try
            {
                ...
                if (iconUrl is null)
                {
                    ...
                    else failures.Add("b:InstallLocation/UninstallString sem executável principal");
                }
                ...
                resolved[index] = package with
                {
                    ...
                    InstallationMethod = InstalledPackageMethodHistory.Get(package.Id)
                };
            }
    ...
    private static bool IsUsableCachedIcon(string path)
    {
    ...
    private static string? FindMainExecutable(UninstallEntry? entry)
    {
        ...
        // O comando de desinstalação costuma apontar para unins*.exe ou setup.exe,
        // cujo ícone raramente representa o aplicativo. Só use esse caminho como
        // último recurso quando o executável tiver relação forte com o nome do app.
        string? uninstallExecutable = TryParseExecutable(entry.UninstallString);
        if (uninstallExecutable is null
            || IsBlacklistedExecutable(Path.GetFileNameWithoutExtension(uninstallExecutable))
            || NameSimilarity(entry.DisplayName, Path.GetFileNameWithoutExtension(uninstallExecutable)) < 0.5)
            return null;

        return uninstallExecutable;
    }
